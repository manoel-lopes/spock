...
from src.equity.usecases.discover_equity_reports import DiscoverEquityReportsUseCase
from src.equity.usecases.submit_equity_analysis import SubmitEquityAnalysisUseCase
...
async def get_submit_equity_analysis_usecase(
    ...
    return SubmitEquityAnalysisUseCase(funds_repo, reports_repo, processing_jobs_repo, job_queue)


async def get_discover_equity_reports_usecase(
    session: AsyncSession = Depends(get_session),
    funds_repo: SqlAlchemyFundsRepository = Depends(get_funds_repository),
    reports_repo: SqlAlchemyReportsRepository = Depends(get_reports_repository),
    report_collector: HttpReportCollector = Depends(get_report_collector),
    submit_usecase: SubmitEquityAnalysisUseCase = Depends(get_submit_equity_analysis_usecase),
) -> DiscoverEquityReportsUseCase:
    return DiscoverEquityReportsUseCase(
