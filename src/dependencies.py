...
from src.domain.application.usecases.calculate_transparency_score.calculate_transparency_score_usecase import (
...
from src.infra.adapters.collectors.implementations.http_report_collector import (
    HttpReportCollector,
)
...
from src.infra.persistence.session import get_session
from src.infra.queue.implementations.celery_job_queue import CeleryJobQueue
...
def get_transparency_analyzer() -> HeuristicTransparencyAnalyzer:
    return HeuristicTransparencyAnalyzer()
...
async def get_submit_analysis_usecase(
    ...
    return SubmitAnalysisUseCase(funds_repo, reports_repo, processing_jobs_repo, job_queue)


async def get_discover_fund_reports_usecase(
    session: AsyncSession = Depends(get_session),
    funds_repo: SqlAlchemyFundsRepository = Depends(get_funds_repository),
    reports_repo: SqlAlchemyReportsRepository = Depends(get_reports_repository),
    report_collector: HttpReportCollector = Depends(get_report_collector),
    submit_usecase: SubmitAnalysisUseCase = Depends(get_submit_analysis_usecase),
) -> DiscoverFundReportsUseCase:
    return DiscoverFundReportsUseCase(
