...
from src.infra.adapters.analysis.implementations.heuristic_transparency_analyzer import HeuristicTransparencyAnalyzer
...
@celery_app.task(
    name="src.workers.tasks.report_analysis.process_report_analysis",
    bind=True,
    max_retries=3,
...
def process_report_analysis(self, data: dict[str, Any]) -> None:
    ...
    try:
        ...
        text = _download_and_extract(
            session, processing_job_id, report_id, pdf_url, force_redownload
        )

        # Analyze
        _analyze_report(session, processing_job_id, report_id, text)
...
def _download_and_extract(
    ...
    extracted = _extract_pdf_sync(pdf_buffer)
...
def _extract_pdf_sync(pdf_buffer: bytes) -> dict[str, Any]:
    ...
    _validate_report_content(text, page_count)
...
def _validate_report_content(text: str, page_count: int) -> None:
...
def _analyze_report(session, job_id: str, report_id: str, text: str) -> None:
...
def _score_report(session, job_id: str, report_id: str) -> None:
    ...
    try:
        ...
        from src.infra.persistence.models import FundModel
        fund = session.get(FundModel, report.fund_id)
        if not fund:
            return

        from src.infra.persistence.models import TransparencyScoreModel
        ...
        from src.infra.persistence.mappers.sqlalchemy_mappers import ReportAnalysisMapper, ReportMapper
